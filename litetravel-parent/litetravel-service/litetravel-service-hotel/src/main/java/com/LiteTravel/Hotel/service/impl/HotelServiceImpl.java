package com.LiteTravel.Hotel.service.impl;

import com.LiteTravel.Hotel.mapper.*;
import com.LiteTravel.Hotel.DTO.HotelDTO;
import com.LiteTravel.Hotel.pojo.Hotel;
import com.LiteTravel.Region.feign.RegionFeign;
import com.LiteTravel.Region.pojo.Region;
import com.LiteTravel.Hotel.service.HotelService;
...
public class HotelServiceImpl implements HotelService {
    ...
    public HotelDTO findById(Integer hotelId) {
        ...
        Region region;
        if((region = regionFeign.findById(hotel.getHotelAddress()).getData()) == null){
            return getHotelDTO(hotel, "Get region failed");
        }
        return getHotelDTO(hotel, region.getMername());
    }
    /*
...
/*        // 查询所有房间
    ...
    private List<HotelDTO> getHotelDTOs(List<Hotel> hotels){
        ...
        Integer[] addressIds = hotels.stream().map(Hotel::getHotelAddress).distinct().toArray(Integer[]::new);
        List<Region> regions = regionFeign.findByIds(addressIds).getData();
        // 使用Map存储地址数据以方便后续赋值
